...
### snpRdata objects
snpRdata objects store a range of variables internally in order to minimize repeat computation of things like allele frequencies. Any caluculated statistics will also be stored into slots. Internally stored data can be accessed via 'slots' using the '@' operator. For example, to see a table of allele frequencies, the geno.tables slot can be accessed:

```{r}
head(my.dat@geno.tables$as)
```

For the most part, snpR is built so that the user never needs to directly access slots. A list of slots can be found in the documentation for import.snpR.data, accessed by help(import.snpR.data).

Almost all snpR functions are built to allow overwriting for easy object management. Since calculated data is stored in slots, snpR functions generally return the snpRdata object that was given to them, but with extra data merged in. For example, to calculate observed heterozygosity, the calc_ho function can be used:
...
Drift effective population size (Ne) can be calculated from LD values using the same methods implemented in Robin Waples' ldne software package. Since LD values between physically linked SNPs will bias these estimates, snpR implements automatic filtering of pairwise LD values to remove comparisons on the same chromosome.

```{r}

ne <- calc_ne(mydat,
              facets = "pop", # calculate Ne for each population
              chr = "group", # the chr arugment names the column in the SNP metadata that contains chromosome IDs.
              pcrit = c(.01, .05, .1), # minor allele frequency cut offs for Ne calcluation. Ne will be calculated at each level.
              conf.level = 0.95) # confidence level for confidence intervals.

ne$ne # the ne data. ne$x holds the input data with added LD values.
...
P-values can also be calculated later using the calc_p_from_bootstraps function.
...
Many functions can run in parallel with the par argument. This relies on the doSNOW parallel backend, and should run on either windows or linux.

```{r, results='hide'}
my.dat2 <- calc_basic_snp_stats(stickSNPs, "group.pop", sigma = 200, step = 50, par = 4)
```

# Other tools and vignettes:

snpR has a a range of other tools for analyzing SNP data. For vignettes on these topics, click the links below:
