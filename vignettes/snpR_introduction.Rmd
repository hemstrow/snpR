...
sample_meta <- data.frame(pop = substr(colnames(stickFORMATs$`0000`)[-c(1:3)], 1, 3), stringsAsFactors = F)

# import
my.dat2 <- format_snps(stickFORMATs$`0000`, input_format = "0000", input_meta_columns = 3, input_mDat = "0000", sample.meta = sample_meta)
```
...
head(get.snpR.stats(my.dat, "pop.chr", stats = "single"))
```
...
plot_clusters also supports a few other plotting tools (UMAP and tSNE), but these are not currently recommended due to reproduceability issues.
...
my.dat <- calc_smoothed_averages(x = my.dat, facets = "pop.chr",
...
```{r}
stats <- get.snpR.stats(x = my.dat, facets = "pop.chr", stats = "single.window")
...
```{r}
stats <- get.snpR.stats(x = my.dat, facets = "pop.chr", stats = "pairwise.window")
...
```{r, results='hide'}
my.dat <- do_bootstraps(x = my.dat, facets = "chr.pop", boots = 1000, sigma = 200, step = 50, statistics = "all", par = 8)
```
```{r}
stats <- as.data.frame(get.snpR.stats(x = my.dat, facets = "chr.pop", stats = "single.window"))
head(stats)
```
...
```{r}
stats <- get.snpR.stats(x = my.dat2, facets = "chr.pop", stats = "weighted.means")
head(stats)
```
