...
sample_meta <- data.frame(pop = substr(colnames(NN[-c(1:3)]), 1, 3), stringsAsFactors = F)

# grab our sample metadata
snp_meta <- NN[,1:3]

# grab the genetic data
gen_data <- NN[,4:ncol(NN)]
...
```{r}
my.dat2 <- read_delimited_snps(NNfile, sample.meta = sample_meta, header_cols = 3)
