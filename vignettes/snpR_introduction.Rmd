...
author: "William Hemstrom and Melissa Jones"
date: "June 27, 2021"
output: 
...
take similar arguments and interpret them in consistent ways. This is done through the use of the snpRdata object class, which all snpR functions take as their first argument. Nearly all functions are written to be overwrite safe:
...
A handful of functions only work with either SNP or sample specific facets, but this is fixed "under the hood." For example, since observed heterozygosity is calculated independently of for each SNP, it makes no sense to calculate it split up by chromosome. Using the "pop.group" facet with calc_ho, therefore, will internally default to "pop".
...
A common first analytical step for new data is to run a PCA to look for any evidence of population structure/sequencing errors. snpR has a function to do this in a streamlined way. We'll also make a t-stochastic neighbor embedding (tSNE) plot as well, and there are a few other dimensionality reduction tools implemented in this package. tSNE is a machine learning method that compresses more PCA axis onto a single two dimensional plot, and can be really nice for showing higher-order structure such as family groups within the sequenced individuals.
...
To run a PCA and tSNE, we can use the plot_clusters function. Note that this function, like all other snpR plotting functions, **does not return a snpRdata object** and we should **not overwrite our data!** so, save it to a new variable.


```{r, results='hide'}
# generate both tSNE and PCA plots
p <- plot_clusters(my.dat, facets = c("pop.fam"), plot_type = c("pca"))
```
...
There are other visualization tools available with plot_clusters (tSNE and UMAP). Both of these approaches attempt to reduce additional data dimensions beyond the first two principal components down to a two-dimensional visualization. Since they require additional optional packages, they are not demonstrated here. Check out the plot_clusters() documentation for details!
...
Each function, like most functions, take a snpRdata object and facets as their first two arguments. Here, we'll use the my.dat object and calculate these stats splitting by both pop and fam and pop simultaneously.

```{r}
# calculate minor allele frequencies, pi, ho, check for private alleles and HWE.
my.dat <- calc_maf(my.dat, c("pop", "fam.pop"))
my.dat <- calc_pi(my.dat, c("pop", "fam.pop"))
my.dat <- calc_ho(my.dat, c("pop", "fam.pop"))
my.dat <- calc_private(my.dat, c("pop", "fam.pop"))
my.dat <- calc_hwe(my.dat, c("pop", "fam.pop"))
...
```{r}
my.dat <- calc_pairwise_fst(x = my.dat, facets = "pop", method = "WC")
```
...
```{r, results='hide'}
# check the number of cores available for this machine and use half of them.
...
```
To view the results, use get.snpR.stats again.
```{r}
