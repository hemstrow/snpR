...
We can view the results with get.snpR.stats as usual! Since the results are a different kind of data, we view them with a different "type" argument (stats = ). For the single (non-pairwise) stats, we can use the "single.window" type: 

```{r}
stats <- get.snpR.stats(x = my.dat, facets = "pop.group", stats = "single.window")
...
```{r}
stats <- get.snpR.stats(x = my.dat, facets = "pop.group", stats = "pairwise.window")
...
```{r}
stats <- get.snpR.stats(x = my.dat, facets = "group.pop", stats = "LD")
...
```{r}
p <- plot_pairwise_LD_heatmap(x = my.dat, facets = "group.pop")
...
```{r}
p <- plot_pairwise_LD_heatmap(x = my.dat, facets = "group.pop", sample.subfacet = "ASP")
...
```{r, results='hide'}
my.dat <- do_bootstraps(x = my.dat, facets = "group.pop", boots = 1000, sigma = 200, step = 50, statistics = "all", par = 8)
```
```{r}
stats <- as.data.frame(get.snpR.stats(x = my.dat, facets = "group.pop", stats = "single.window"))
head(stats)
```
...
# my.dat2 <- calc_basic_snp_stats(x = stickSNPs, facets = "group.pop", sigma = 200, step = 50, par = 8)
...
```{r}
stats <- get.snpR.stats(x = my.dat2, facets = "group.pop", stats = "weighted.means")
head(stats)
```
