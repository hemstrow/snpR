...
* A PCA cannot be constructed with missing data, so we interpolate with the average number of each allele across all individuals as the default ("bernoilli"). There are other interpolation methods available in this package (see plot_clusters documentation for more details). 
* Filtering data to remove poorly sequenced individuals and loci before PCA is recommended. 
* tSNE and UMAP require installation of additional libraries in order to run.
...
snpR has a workflow to export snpRdata objects for parentage assignment using COLONY (Jones and Wang, 2010) or running natively in R using Sequoia (Huisman, 2017). COLONY has a robust clustering algorithm, but requires more knowledge about the sample set and manipulation of data to run the analyses. It is a little more challenging to use with datasets that could comprise multiple generations. Sequoia is a great option for quickly running multi-generational samples with less known metadata.

#### COLONY
Formatting data to run in COLONY can be difficult with complicated datasets, but snpR can help make the process easier. The function write_colony_input takes a snpR object and writes a file that can be pulled into the commandline based COLONY interface.

write_colony_input writes a text file formatted for the COLONY commandline interface which is saved in a folder named 'colony' in your working directory. If the folder does not already exist, one will automatically be created. 

If you do not split out data into paternal, maternal, and offspring groups before running the write_colony_input function, COLONY will run analyses as if all samples are potential siblings. If you have known males, females, offspring, or at least birth years, you can manually split out the data using subset_snpRdata. 
...
```{r, echo=FALSE, eval=FALSE}
# example to produce COLONY input file for all individuals as potential siblings
write_colony_input(x = stickSNPs, outfile = "sticksnps1.dat", sampleIDs = .sample.id, seed = 4562)
```

Preparing runs in COLONY with potential parents is more complicated and requires preparation.
...
```
#### Sequoia
Another option is to use Sequoia! The function run_sequoia is designed to take a snpR data object and run parentage analysis and pedigree construction. There are additional arguments and parameters that can be added in the base run_sequoia function (see the Sequoia and run_sequoia documentation for more details). 

Sequoia requires use of snps with high minor allele frequency (~0.3 or more), snpR objects can be filtered in the command directly! Do note that SNPs that are in fewer than 50% of individuals will be automatically dropped by sequoia.

Now let's go through an example of parentage and pedigree construction with sequoia.
```{r}
# actually going to run this since native in R and relatively quick to complete
...
stk.par <- run_sequoia(x = stk, facets = "pop", run_dupcheck = FALSE, run_parents = TRUE, run_pedigree = FALSE, min_maf = 0.3) # sequoia requires use of snps with high minor allele frequency
