...
LD_full_pairwise_old <- function(data, num_start, prox_table = FALSE, matrix_out = TRUE) {
  i <- 1
  require(dplyr)
...
Full_LD_w_groups_old <- function(data, num_start, prox_table = FALSE, matrix_out = TRUE, report = 1){
...
Full_LD_g_par_old <- function(data, num_start, num_cores, prox_table = FALSE, matrix_out = TRUE){
  if (prox_table == FALSE & matrix_out == FALSE){
    stop("Please specify output format.")
  }
  require(doParallel)
...
LD_full_pairwise <- function(x, ecs, prox_table = TRUE, matrix_out = TRUE, mDat = "N") {
  ...
  tabulate_haplotypes <- function(x, y){
    ...
    ##figure out the locus names for locus one.
    #l1s <- unique(c(substr(substr(gl, 1, sform*2),1,sform),
    #                substr(substr(gl, 1, sform*2),sform+1,sform*2))) #what are the alleles for locus 1
    
    ##figure out locus names for locus two
    #l2s <- c(ifelse(yi %in% As, colnames(amat)[1], NA), 
    #         ifelse(yi %in% Bs, colnames(amat)[2], NA),
    #         ifelse(yi %in% Cs, colnames(amat)[3], NA),
    #         ifelse(yi %in% Ds, colnames(amat)[4], NA)) #vector of allele p/a
      
    #l2sm <- t(matrix(l2s,4, length(yi), T)) #put this into a more sensical form.
    #l2ac <- rowSums(ifelse(is.na(l2sm), T, F)) #get the allele count at each locus
    #if(any(l2ac > 2)){ #sanity check
    #  stop("More than two alleles detected at locus:", yi, "\n")
    #}
    #l2sm <- cbind(l2sm, NA) #add a column to be a placeholder if a locus only has one allele.
    #l2sm[,5] <- ifelse(l2ac  == 1, "miss", NA) #fill this placeholder with a note if only one allele.
    #l2s <- as.vector(t(l2sm)) #revectorize
    #l2s <- na.omit(l2s) #remove NAs.
    #l2sm <- matrix(l2s, 2, length(yi), T) #put back into a matrix, now with only two columns for allele one and allele two at each locus.
    #remove(l2s, l2ac) #clean up.
