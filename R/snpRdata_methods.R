...
setMethod("show", "snpRdata", function(object) {
  ...
  base_facet_id <- x@.facet.id.key$.facet.id[which(x@.facet.id.key$facet == ".base")]
  mafs <- object@stats[which(object@stats$.facet.id == base_facet_id),]$maf
  ...
  cat(ifelse(bi_allelic, "bi-allelic", "non bi-allelic"), methods::is(object)[1], "with", nrow(object), "SNPs and", ncol(object), "samples.\n",
      ...
      "Percent missing data:", mean(1 - (Matrix::rowSums(object@geno.tables$gs[which(object@facet.meta$.facet.id == base_facet_id),, drop = F])/nrow(object@sample.meta))), "\n",
      .console_hline(), "\n",
