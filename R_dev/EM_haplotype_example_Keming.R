...
while(n <= nrow(ex)){
  ...
  haptable <- hap.ex[n,] # these are the counts from the unambigious loci
  ## haps <- c("AC", "AG", "GC", "GG") # Is this universal or only for line 1?
  ## names(haptable) <- haps
  ...
  single_haplotype_estimation <- function(x, haptable, sigma = 0.0001){
    ...
    while(diff > sigma){
      ...
      n1hap.freqs[c(1, 4)] <- n1hap.freqs[c(1, 4)] + (x[4]*op1.e*.5) # we basically add the expected number of haplotypes for the double heterozygotes
      n1hap.freqs[c(2, 3)] <- n1hap.freqs[c(2, 3)] + (x[4]*op2.e*.5)
